...
| 4.1.2 | | | 페이지 상단의 프로젝트 생성 버튼을 눌러 새로운 프로젝트를 생성할 수 있다. | | | | 10.2.5 | H | | 
...
| 4.3.3 | | | 페이지 상단의 검색 버튼을 눌러 참여 코드 입력 후 프로젝트 관리자에게 참여 요청을 보낸다. | | | | 10.2.6 | H | | 
...
| 6 | 시간조율 | | | | | | - | - | | 
| 6.1 | 시간조율 표 생성 | 사용자들 간의 시간을 조율하기 위한 표를 생성한다. | | | | | - | - | | 
...
| 6.1.4 | | | 생성할 시간조율 표의 형태를 만들기 위해 요구하는 정보를 입력 및 선택하고 생성 버튼을 누른다. | | | | | H | | 
| 6.1.5 | | | 열은 한 번에 최소 1개, 최대 7개까지 생성 가능하다. | | | | | H | | 
| 6.1.6 | | | 시작 시간대와 종료 시간대는 24시간 모두 선택 가능하다. | | | | | H | | 
| 6.1.7 | | | 시간조율 표의 이름을 입력하지 않았거나 열 생성 개수가 선택되지 않은 경우, 오류 메시지를 출력한다.  | | | | | H | | 
| 6.1.8 | | | 시간조율 표의 이름을 입력하지 않았거나 열 생성 개수가 선택되지 않은 경우, 재입력 받도록 한다. | | | | | H | | 
| 6.1.9 | | | 시간조율 표의 이름은 한 번 생성되면 수정할 수 없다. | | | | | H | | 
| 6.1.10 | | | 생성된 모든 시간조율 표는 아래의 리스트에서 확인할 수 있다. | | | | | H | | 
...
| 6.1.16 | | | 시간조율 표의 왼쪽은 자신의 시간을 편집할 수 있는 공간, 오른쪽은 자신의 시간과 다른 사용자들의 시간 정보를 가져와 보이는 공간이다. | | | | | H | | 
...
| 6.2.3 | | | 시간조율 표가 하루 이상 생성된 상태여야 한다. | | | | 6.1.4 | H | | 
| 6.2.4 | | | 편집할 시간조율 표를 리스트에서 선택한다. | | | | | H | |
...
| 6.3.3 | | | 사용자는 시간조율 표를 하루 이상 생성한 상태여야 한다. | | | | 6.1.4 | H | | 
