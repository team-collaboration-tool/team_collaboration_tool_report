...
| 1.1.4 | | | 회원가입을 하기 위해 요청하는 정보를 입력하고, 회원가입 버튼을 누른다. | | | | | H | | 
| 1.1.5 | | | 사용자의 정보가 양식과 다르거나 필수 항목에 아무것도 입력하지 않은 경우, 오류 메시지를 출력한다. | | | | | H | | 
| 1.1.6 | | | 사용자의 정보가 양식과 다르거나 필수 항목에 아무것도 입력하지 않은 경우, 재입력 받도록 한다. | | | | | H | | 
| 1.1.7 | | | 사용자의  아이디(이메일)가 다른 사용자와 중복되는 경우, 오류 메시지를 출력한다. | | | | | H | | 
| 1.1.8 | | | 사용자의 아이디(이메일)가 다른 사용자와 중복되는 경우, 재입력 받도록 한다. | | | | | H | | 
| 1.1.9 | | | 회원가입에 성공하면, 사용자의 정보가 DB에 등록되며 웹사이트에 로그인할 수 있다. | | | | | H | | 
| 2 | 로그인 | | | | | | - | - | |  
| 2.1 | | 웹사이트의 서비스를 사용하기 위해 로그인이 필요하다. | | | | | - | - | | 
| 2.1.1 | | | 사용자는 회원가입을 제외한 모든 기능을 사용하기 위해서 로그인을 해야 한다. | | | | | H | | 
| 2.1.2 | | | 아이디, 비밀번호를 입력하여 로그인을 시도한다. | | | | | H | | 
| 2.1.3 | | | 아이디나 비밀번호가 일치하지 않거나 아무것도 입력하지 않은 경우, 오류 메시지를 출력한다. | | | | | H | | 
| 2.1.4 | | | 아이디나 비밀번호가 일치하지 않거나 아무것도 입력하지 않은 경우, 재입력 받도록 한다. | | | | | H | | 
| 2.1.5 | | | 사용자 정보가 존재하지 않을 경우, 오류 메시지를 출력한다. | | | | | H | | 
