...
**Project description:** 본 프로젝트 '협업의 민족'은 현대 팀 협업 환경에서 발생하는 비효율적인 일정 조율, 의사소통, 그리고 개인 일정과의 동기화 부재로 인한 누락 및 충돌 문제를 해결하기 위해 제안된 웹 기반 협업 관리 도구이다. 이 시스템은 별도 설치 없이 접근 가능한 웹 환경과 직관적 UI를 바탕으로, 팀 전체 스케줄과 개인 일정을 통합 관리하는 '대시보드 및 달력', 팀원 간 가용 시간 교집합을 계산하여 회의 시간을 효율적으로 확정하는 '시간조율', 그리고 아이디어 공유와 자료 교환, 피드백 순환을 위한  '게시판' 기능을 핵심으로 제공한다. '협업의 민족'은 이러한 통합적 기능들을 단일 플랫폼 안에서 제공함으로써, 사용자에게 보다 편리하고 효율적인 일정 관리 및 원활한 소통 환경을 제공하는 것을 최종 목적으로 한다.
...
| 4.3.3 | | | 페이지 상단바의 검색 버튼을 눌러 참여코드 입력 후 프로젝트 관리자에게 참여 요청을 보낸다. | | | | 12.1.3 | H | | 
...
| 7.2.8 | | | 해당 일정의 날짜가 지나면 자동으로 대시보드에서 삭제된다. | | | | | H | | 
...
| 7.6 | 외부 웹사이트 접속 | 추가한 바로가기를 사용하여 외부 웹사이트에 접속한다. | | | | | - | - | | 
...
| 9.3.2 | | | 자신의 전공이나 직무, 관심분야가 변경된 경우 사용한다. | | | | | H | | 
| 9.3.3 | | | 수정 버튼을 눌러 카테고리에서 변경할 전공/직무를 선택한다. | | | | | H | | 
| 9.3.4 | | | 변경하기 버튼 눌러 정보를 변경한다. | | | | | H | | 
| 9.3.5 | | | 변경이 성공하면, 사용자 정보가 DB에 업데이트된다. | | | | | H | | 
| 9.4 | 회원 탈퇴 | 웹사이트 사용을 더 이상 사용하지 않을 사용자들은 회원을 탈퇴한다. | | | | | - | - | | 
| 9.4.1 | | | 사용자는 설정 페이지에 접근한 상태여야 한다. | | | | 9.1.2 | H | | 
| 9.4.2 | | | 회원을 탈퇴하고자 할 경우, 사용자가 작성한 글은 남아있게 되며 유저 정보만 삭제된다. | | | | | H | | 
| 9.4.3 | | | 회원 탈퇴 버튼을 누른다. | | | | | H | | 
| 9.4.4 | | | 사용자는 탈퇴를 하기 위해 현재 비밀번호를 다시 입력해야 한다. | | | | | H | | 
| 9.4.5 | | | 비밀번호가 일치할 경우, 사용자는 회원 탈퇴에 성공한다. | | | | | H | | 
| 9.4.6 | | | 회원 탈퇴에 성공하면, 사용자의 정보는 DB에서 삭제된다. | | | | | H | | 
| 9.4.7 | | | 비밀번호가 일치하지 않거나 아무것도 입력하지 않은 경우, 오류 메시지를 출력한다. | | | | | H | | 
| 9.4.8 | | | 회원 탈퇴한 사용자는 필요하다면 다시 회원 가입할 수 있다. | | | | | H | | 
| 9.5 | 이름 변경 | 회원가입 시 등록한 이름을 변경한다. | | | | | - | - | | 
| 9.5.1 | | | 사용자는 설정 페이지에 접근한 상태여야 한다. | | | | 9.1.2 | H | | 
| 9.5.2 | | | 자신의 이름이 변경된 경우 사용한다. | | | | | H | | 
| 9.5.3 | | | 수정 아이콘을 눌러 변경할 이름을 작성한다. | | | | | H | | 
| 9.5.4 | | | 변경하기 버튼 눌러 정보를 변경한다. | | | | | H | | 
| 9.5.5 | | | 변경이 성공하면, 사용자 정보가 DB에 업데이트된다. | | | | | H | |
| 9.6 | 전화번호 변경 | 회원가입 시 등록한 전화번호를 변경한다. | | | | | - | - | | 
| 9.6.1 | | | 사용자는 설정 페이지에 접근한 상태여야 한다. | | | | 9.1.2 | H | | 
| 9.6.2 | | | 자신의 전화번호가 변경된 경우 사용한다. | | | | | H | | 
| 9.6.3 | | | 수정 아이콘을 눌러 변경할 전화번호를 작성한다. | | | | | H | | 
| 9.6.4 | | | 변경하기 버튼 눌러 정보를 변경한다. | | | | | H | | 
| 9.6.5 | | | 변경이 성공하면, 사용자 정보가 DB에 업데이트된다. | | | | | H | | 
| 10 | 사이드바 | | | | | | - | - | | 
