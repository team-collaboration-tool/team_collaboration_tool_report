...
| 4 | 활성화된 삭제 버튼을 누를 경우, 해당 프로젝트는 DB에서 삭제된다. |
...
| | 4b. 프로젝트에 관리자를 제외한 다른 멤버가 남아있는 경우 |
| | 4b.1. 시스템은 프로젝트에 멤버가 있어 삭제할 수 없다는 오류 메시지를 출력한다. |
| | 4b.2. 관리자는 멤버 목록에서 ‘X' 버튼을 눌러 멤버를 모두 삭제한다. |
| | 4b.3. 프로젝트 삭제 버튼 누르는 단계로 이동한다. (Use case #11-1) |
...
| | 3a.3. 사용자는 투표 제목, 선택지, 마감 날짜 등을 작성한다. |
...
| | 3e. 투표 선택지 개수가 조건에 맞지 않을 경우 |
| | 3e.1. 시스템은 오류메시지를 출력한다. |
| | 3e.2. 투표 정보 입력 단계로 돌아간다. (Use case #17-3a.3) |
...
| **Status** | Analysis |
| **Primary Actor** | 프로젝트 참여자 |
| **Preconditions** | 프로젝트의 시간조율 페이지에 접근한 상태여야 하고, 생성된 시간조율표가 존재해야 하며, 해당 표의 마감 날짜가 지나지 않은 상태여야 한다. |
...
| Step | Branching Action |
| :--- | :--- |
| 2 | 2a. 선택한 시간조율표의 마감 날짜가 지난 경우 |
| | 2a.1. 시스템은 해당 시간조율표의 정보를 삭제한다. |
| | 2a.2. 사용자는 리스트에서 해당 표를 확인할 수 없다. |
...
| **Trigger** | 로그아웃 버튼을 누르거나 웹 브라우저의 창을 닫을 때 |
...
| Step | Branching Action |
| :--- | :--- |
| 1 | 1a. 사용자가 웹 브라우저 창을 닫을 경우 |
| | 1a.1. 시스템은 브라우저 종료 이벤트를 감지한다. |
| | 1a.2. 별도의 로그아웃 확인 절차 없이 즉시 세션을 종료하고 토큰을 삭제한다. |
