...
| 3 | 사용자는 아이디(이메일)를 입력하고, 중복 확인을 요청한다. |
| 4 | 시스템은 해당 이메일이 사용 가능함을 확인하고, 사용자에게 알린다. |
| 5 | 사용자가 비밀번호와 비밀번호 확인란에 값을 입력한다. |
| 6 | 시스템은 두 입력이 일치함을 확인하고, 오류가 없으면 다음 버튼을 활성화 한다. |
| 7 | 사용자가 다음 버튼을 누른다. |
| 8 | 시스템은 회원가입에 필요한 정보(이름, 전화번호, 전공/직무, 개인정보 수집 동의)를 표시한다. |
| 9 | 사용자는 이름, 전화번호를 입력하고 개인정보 수집 동의에 체크한다. |
| 10 | 시스템은 필수 항목이 모두 충족되었는지 확인하고, 모두 충족되었다면 회원가입 버튼을 활성화한다. |
| 11 | 사용자는 회원가입 버튼을 누른다. |
| 12 | 시스템은 사용자 정보를 저장하고 계정 생성을 완료한다. |
...
| 6 | 6a. 두 비밀번호가 일치하지 않는 경우 |
| | 6a.1. 시스템은 오류 메시지를 출력한다. |
| | 6a.2. 다음 버튼은 비활성화 상태를 유지한다. |
| | 6a.3. 비밀번호 입력 단계로 돌아간다. (Use case #1-5) |
| 10 | 10a. 필수 항목을 누락한 경우 |
| | 10a.1. 시스템은 오류 메시지를 출력한다. |
| | 10a.2. 회원가입 버튼은 비활성화 상태를 유지한다. |
| | 10a.3. 회원가입 정보 입력 단계로 돌아간다. (Use case #1-9) |
...
| 3 | 시스템은 프로젝트를 생성하고 영어와 숫자가 랜덤으로 섞인 6자리 참여 코드를 부여한다. |
...
| 2 | 사용자는 참여하고자 하는 프로젝트의 참여 코드를 입력한다. |
| 3 | 시스템은 참여 코드가 존재하는지 확인 후 프로젝트명을 화면에 출력한다. |
