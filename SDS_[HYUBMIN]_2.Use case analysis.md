...
| | |
| :--- | :--- |
| **Summary** | 프로젝트 관리 페이지에서 생성된 참여 코드를 확인한다. |
| **Scope** | 협업의 민족 |
| **Level** | User level |
| **Author** | 박한비 |
...
| **Success Post Condition** | 사용자가 프로젝트 참여 코드를 확인한다. |
| **Failed Post Condition** | 시스템이 참여 코드 불러오기에 실패한다. |

#### MAIN SUCCESS SCENARIO
| Step | Action |
| :--- | :--- |
| S | 사용자가 프로젝트 참여 코드를 확인한다. |
| 1 | 사용자는 프로젝트 관리 페이지에 접근한다. |
| 2 | 시스템은 DB에서 해당 프로젝트의 참여 코드를 불러온다. |
| 3 | 시스템은 불러온 참여 코드를 화면에 표시한다. |
| 4 | 사용자가 코드를 확인하면 끝난다. |
...
| | |
| :--- | :--- |
| **Summary** | 프로젝트 참여 요청을 승인 또는 거절하거나, 참여 중인 멤버를 삭제하거나 관리자 권한을 양도한다. |
| **Scope** | 협업의 민족 |
| **Level** | User level |
| **Author** | 박한비 |
| **Last Update** | 2025. 11. 3. |
| **Status** | Analysis |
| **Primary Actor** | 프로젝트 관리자 |
| **Preconditions** | 사용자는 해당 프로젝트의 관리자 권한을 가지고, 프로젝트 관리 페이지에 접근한 상태여야 한다. <br> 또한, 승인 대기 중인 사용자가 최소 1명 존재하거나, 관리자를 제외한 참여 멤버가 최소 1명 존재해야 한다. |
...
| | 2d. 관리자 권한 넘기기 아이콘을 선택한 경우 (프로젝트 멤버 관리) |
| |  2d.1. 관리자 권한이 해당 멤버에게 넘겨진다. |
...
| |  3b.4. 필요하다면 중복 선택, 익명 투표 설정도 가능하다. |
| |  3b.5. 투표 선택지는 최소 2개부터 최대 8개까지 입력 가능하다. |
| |  3b.6. 투표 생성 버튼을 눌러 투표를 생성한다. |
| | 3c. 투표가 생성되지 않는 경우 |
...
| |  3d.3. 이미 10개가 등록되어 있는 경우, 공지사항 유무 체크 박스가 비활성화되며, 안내 문구가 출력된다. |
...
| |  4a.6. 이미 투표를 한 상태이고 마감 전이면 투표하기 버튼을 누른다. |
...
| | 1b. 표의 열 생성 개수 |
| |  1b.1. 한 번에 최소 1개, 최대 7개까지 생성 가능하다. |
| | 1c. 시작 날짜 |
| |  1c.1. 오늘 날짜로 기본 선택되어 있다. |
| 2 | 2a. 이름이 입력되지 않은 경우 |
...
| 4 | 시스템은 DB에 편집 정보를 저장하고, 오른쪽 표에도 반영한다. |
| 5 | 사용자가 편집 결과를 확인하면 끝난다. |

#### EXTENSION SCENARIOS
| Step | Branching Action |
| :--- | :--- |
| 4 | 4a. 시간 일정을 추가하는 경우 |
| |  4a.1. 표의 빈 부분 위에서 드래그를 시작해야 한다. |
| | 4b. 시간 일정을 삭제하는 경우 |
| |  4b.1. 이미 채워진 부분 위에서 드래그를 시작해야 한다. |
| 5 | 5a. 정보를 반영할 경우 |
| |  5a.1. 다른 사용자가 보는 시간조율표에도 공유된다. |
...
| 4 | 사용자가 종합 시간표를 확인하면 끝난다. |

#### EXTENSION SCENARIOS
| Step | Branching Action |
| :--- | :--- |
| 3 | 3a. 표 형식 |
| |  3a.1. 정각 단위는 선으로, 30분 단위는 점선으로 표시된다. |
| | 3b. 정보 제공 범위 |
| |  3b.1. 다른 사용자의 상세 정보는 표시되지 않고, 겹치는 인원수만 색 농도로 제공된다. |
...
| |  4a.2. 비밀번호를 입력하는 단계로 돌아간다. (Use case #29-3) |
