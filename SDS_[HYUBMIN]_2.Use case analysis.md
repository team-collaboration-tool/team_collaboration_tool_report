...
| 3 | 사용자는 정보를 입력하고 다음 버튼을 누른다. |
| 4 | 시스템은 회원가입에 필요한 정보(이름, 전화번호, 전공/직무, 개인정보 수집)를 표시한다. |
| 5 | 시스템은 필수 항목이 모두 충족되었는지 확인하고, 모두 충족되었다면 회원가입 버튼을 활성화한다. |
| 6 | 사용자는 회원가입 버튼을 누른다. |
| 7 | 시스템은 사용자 정보를 저장하고 계정 생성을 완료한다. |
